...
export const GroupSection: React.FC<GroupSectionProps> = ({
    ...
    if (dragItem) {
      ...
      if (!isSameGroup) {
        if (dragItem.type === 'group') {
          ...
          const isTopPortion = e.clientY < topThreshold;
          if (isTopPortion) {
            setDropTarget({
              type: 'window',
              id: group.windowId,
  ...
  const handleDragLeave = useCallback(() => {
    ...
    if (dropTarget?.type === 'window' && dropTarget?.windowId === group.windowId && dropTarget?.index === groupIndex) {
      setDropTarget(null);
    }
  }, [dropTarget, group.id, group.windowId, groupIndex, setDropTarget]);
  ...
  const isDropTargetBeforeGroup = dropTarget?.type === 'window' && dropTarget?.windowId === group.windowId && dropTarget?.index === groupIndex && dragItem?.type === 'tab';
  const isDropTargetTop = (isDropTargetForGroup && dropTarget?.index === groupIndex) || isDropTargetBeforeGroup;
